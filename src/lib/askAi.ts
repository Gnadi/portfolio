 ...
 *
 * The prompt runs to about 1.6 KB once encoded. Repeating it inside five
 * hrefs put roughly 8 KB of duplicated text into every page, so each link
 * ships only its `base` — the provider URL up to and including the query key —
 * and a few lines of inline script append the prompt, which the bar carries
 * once. Without JavaScript the links still open the assistant; they just
 * arrive without the prompt prefilled.
 */
...
export interface AiProvider {
	...
	glyph: Glyph;
	/**
	 * The provider URL up to and including the prompt's query key, so the
	 * URI-encoded prompt can simply be appended.
	 */
	base: string;
}
